package com.r3.corda.lib.settler.swift.services

import com.fasterxml.jackson.module.kotlin.jacksonObjectMapper
import com.fasterxml.jackson.module.kotlin.readValue
import com.github.kittinunf.fuel.httpGet
import com.github.kittinunf.fuel.httpPost
import com.r3.corda.lib.settler.swift.types.*
...
class SWIFTClient(
    ...
    private fun getUnsignedPayload(uetr : String) : SWIFTUnsignedPayload {
        ...
        logger.info(messageWithParams("Getting unsigned payload", "UETR" to uetr))
        val (_, res, _) = url
                .httpGet()
                .header("x-api-key" to apiKey)
                .header("accept" to "application/json")
        ...
        if (res.statusCode >= 400) {
            ...
            val mapper = jacksonObjectMapper()
            logger.info(httpResultMessage("Successfully retrieved unsigned payment payload", res.statusCode, responseData, "UETR" to uetr))
    ...
    fun getPaymentStatus(uetr : String) : SWIFTPaymentStatus {
        ...
        logger.info(messageWithParams("Getting payment status", "UETR" to uetr))
        ...
        if (res.statusCode >= 400) {
            ...
            logger.info(httpResultMessage("Successfully retrieved payment status", res.statusCode, responseData, "UETR" to uetr))
            return mapper.readValue(responseData)
        }
    }
    ...
    fun updatePaymentStatus(uetr : String, status : SWIFTPaymentStatusType, confirmedAmount: String) {
        ...
        logger.info(messageWithParams("Updating payment status.", "UETR" to uetr))
        val (_, res, _) = checkStatusUrl
                .httpPost()
                .header("x-api-key" to apiKey)
                .header("accept" to "application/json")
        ...
        if (res.statusCode >= 400) {
            ...
            logger.info(httpResultMessage("Successfully updated payment status", res.statusCode, responseData, "UETR" to uetr))
        }
    }
